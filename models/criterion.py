...
class SetCriterion(nn.Module):
    def __init__(self, num_classes, weight_dict, eos_coef, losses, group_losses=None,
        ...
        if self.dataset == 'cafe':
            empty_weight[-1] = eos_coef
            empty_group_weight[-1] = args.group_eos_coef
        elif self.dataset == 'social_cad':
            empty_weight[0] = eos_coef
            empty_group_weight[0] = args.group_eos_coef
    ...
    def loss_labels(self, outputs, targets, num_boxes, log=True):
        ...
        for batch_idx in range(src_logits.shape[0]):
            ...
            if src_logits_log is None:
                ...
                src_logits_log = torch.cat([src_logits_log.squeeze(0), src_logit.squeeze(0)], dim=0)
                tgt_classes_log = torch.cat([tgt_classes_log.squeeze(0), target_class.squeeze(0)], dim=0)
    ...
    def loss_group_code(self, outputs, targets, group_indices, log=True):
        ...
        for batch_idx in range(sim.shape[0]):
            ...
            # Dummy boxes are actions that are not associated with any group (NA)
            if self.dataset == 'cafe':
                loss_membership += F.binary_cross_entropy(sim_batch, target_members_batch)
            elif self.dataset == 'social_cad':
                # Create mask to ignore group id == -1 values, besides the dummy boxes, here mask out the singletons.
                valid_mask = sim_batch > -1

                if valid_mask.sum() > 0:  # Only compute loss if there are valid elements
                    sim_valid = sim_batch[valid_mask]
                    target_valid = target_members_batch[valid_mask]
                    loss_membership += F.binary_cross_entropy(sim_valid, target_valid)
        ...
        for batch_idx in range(actor_embeds.shape[0]):
            ...
            for actor_idx in range(N):
                ...
                if group_id > -1:
                    ...
                    if self.dataset == 'social_cad' and len(positive_idx) == 0:
                        continue
                    ...
                    batch_valid_actors += 1

            total_valid_actors += batch_valid_actors

            # For cafe dataset, normalize by batch actors count
            if self.dataset == 'cafe' and batch_valid_actors > 0:
                consistency_loss /= batch_valid_actors

        # Final normalization
        if self.dataset == 'social_cad':
            # Average over all valid actors across all batches
            if total_valid_actors > 0:
                consistency_loss /= total_valid_actors
        elif self.dataset == 'cafe':
        ...
        for tgt in targets:
            ...
            for box_idx in range(num_clip_boxes):
                if self.dataset == 'cafe':
                    ...
                    if bool(tgt["actions"][0, box_idx] == 0):
                        tgt["dummy_idx"][0, box_idx] = 0
