...
# Backbone parameters
parser.add_argument('--backbone', default='base', choices=['base', 'finetune'], type=str)
...
parser.add_argument('--hidden_dim', default=768, type=int, help='transformer channel dimension')
parser.add_argument('--use_group_prompts', action='store_true', help='Use prompts strategy for backbone')

# RoI Align parameters
parser.add_argument('--num_boxes', default=14, type=int, help='maximum number of actors')
parser.add_argument('--crop_size_height', default=1, type=int, help='roi align crop size')
parser.add_argument('--crop_size_width', default=1, type=int, help='roi align crop size')

# Group Transformer
parser.add_argument('--group_transformer_arch', type=str, default='group_transformer',
                    help='Choose group transformer architecture')

parser.add_argument('--gar_nheads', default=4, type=int, help='number of heads')
parser.add_argument('--gar_enc_layers', default=1, type=int, help='number of group transformer layers')
parser.add_argument('--gar_ffn_dim', default=1536, type=int, help='feed forward network dimension')
parser.add_argument('--use_mask', action='store_true', help='use mask attention')
parser.add_argument('--num_group_tokens', default=7, type=int, help='number of group tokens')
...
parser.add_argument('--backbone_lr_ratio', default=0.1, type=float, help='backbone learning rate ratio to decoder')
...
parser.add_argument('--optimizer', default='adamw', type=str, choices=['sgd', 'adam', 'adamw'], help='optimizer')
...
parser.add_argument('--print_model', default=False)
...
def main():
    ...
    parameters = 'Number of full model parameters: {}'.format(sum([p.data.nelement() for p in model.parameters() if p.requires_grad]))
    ...
    if args.dataset == 'cafe':
        ...
        metrics = evaluation.SocialCAD_Evaluation(args)

    # training phase
    for epoch in range(start_epoch, args.epochs + 1):
        print_log(save_path, '----- %s at epoch #%d' % ("Train", epoch))
        train_log = train(train_loader, model, criterion, optimizer, epoch, args.dataset)
        ...
        if epoch % args.test_freq == 0:
            ...
            test_log, result = validate(test_loader, model, criterion, metrics, epoch, args.dataset)
            ...
            if args.dataset == 'cafe':
                ...
                print_log(save_path, "Individual Action Accuracy: %.4f" % result['Individual_Action_Accuracy'])
                print_log(save_path, "Membership Accuracy: %.4f" % result['Membership_Accuracy'])
                print_log(save_path, "Social Accuracy: %.4f" % result['Social_Accuracy'])
...
def train(train_loader, model, criterion, optimizer, epoch, dataset):
    model.train()
    criterion.train()
    ...
    for i, (images, targets, infos) in enumerate(metric_logger.log_every(train_loader, print_freq, header)):
        ...
        elif dataset == 'social_cad':
            dummy_mask = torch.stack([t['actions'] == 0 for t in targets]).squeeze()
        num_batch = images.shape[0]
        num_frame = images.shape[1]
        print(f'Batch {i+1}/{print_freq} ({(i+1)/print_freq*100:.1f}%)')

        # compute output
        outputs = model(images, boxes, dummy_mask)
        loss_dict = criterion(outputs, targets, log=False)

        # Calculate total loss with updated weights
        ...
        loss_dict_reduced_scaled = {k: v * criterion.weight_dict[k]
                                    for k, v in loss_dict_reduced.items() if k in criterion.weight_dict}
...
@torch.no_grad()
def validate(test_loader, model, criterion, metrics, epoch, dataset):
    model.eval()
    criterion.eval()
    ...
    if dataset == 'cafe':
        ...
        name_to_vid = {name: i + 1 for i, name in enumerate(SEQS_SOCIAL_CAD)}
    else:
